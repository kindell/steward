...
# PRODUCTION 2026-09-12 14:05 (the owner's home on a Linux host, eleven rows on one login): without this question the
# first M7 refused every spawn as soon as ANY sibling failed to observe in the round - a sibling called
# 'Point' cannot hold 'Parent→Project' whether it is observable or not, and a session the owner had asked
