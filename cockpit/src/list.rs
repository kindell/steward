...
mod tests {
    ...
    fn fleet(sessions: Vec<Session>, hidden: u32) -> Fleet {
        Fleet { viewer: "alice".to_string(), sessions, hidden, unreadable: vec![], hub: "h".to_string() }
    }
    ...
    fn unreadable_names_are_on_screen() {
        let f = Fleet { viewer: "alice".to_string(), sessions: vec![], hidden: 0, unreadable: vec!["broken-a".into(), "broken-b".into()], hub: "h".to_string() };
    ...
    fn unreadable_hidden_and_overflow_together_keep_the_accounting_lines() {
        let sessions: Vec<Session> = (0..5)
            .map(|i| sess(&format!("s{i}"), &[], "up", None))
            .collect();
        let f = Fleet { viewer: "alice".to_string(), sessions, hidden: 2, unreadable: vec!["broken-a".into()], hub: "h".to_string() };
