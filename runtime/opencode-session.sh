...
if [ "$_mates_rc" -eq 78 ]; then
  ...
  echo "opencode-session: DEGRADED — the register refused another row's identity claim while reading the mates (rc 78, cause above); measured again just now, this session's own row $_own_reads, so it starts with an incomplete mate list" >&2
