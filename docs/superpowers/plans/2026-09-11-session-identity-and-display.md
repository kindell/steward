...
**9a and 9b are BUILT** (registry: `registry_display_of_fields`, `registry_session_login_key`, `registry_session_rendered_unique`, `registry_session_work_rule`; wired in `_registry_validate_session_stage` inside the write lock and in `linux/hub/enroll` after its same-home label gate; supervisor: `host_display_reserved`, `host_gate_refuses` before the claim in a spawn, before the keyed no-process spawn, and before the rename baseline). Tests: registry-session-add 102 (section 10), registry-session-display 71 (section 9: the predicates), hub-enroll-conf 109 (a2), session-rc-label-unique 19, supervisor-bridge 223 (section 39). Mutations: no rendered gate, no work rule, dead row reserves, no gate before spawn, no gate before rename, strict ignored — all bite. Measured on the way: in `session add` the rendered gate fires before the work rule (the same project renders the same display), and in enroll the (account, slug) gate speaks first because the naming convention makes a same-project request the same slug; the work rule is therefore proven as a predicate and stands behind both as defence in depth. 9c (retarget refusal) is not built. Task 6 is deployable with respect to the gates once the advisor has locked 9a/9b.
...
### Tasks 9–12 — OUTLINES (as v3/v4; fifteen fields; census via `--bootstrap`; ACCOUNT migration named as P0 prerequisite). Task 9 is the host gate named above.
