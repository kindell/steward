...
- Refusal reasons (field 9, answer `unknown` unless noted): `row-does-not-load` `account-missing` `account-does-not-resolve` `owner-uid-unknown` `other-owner` (answer `uninspectable`) `login-account-mismatch` `config-dir-unreadable` `sessions-dir-unreadable` `tmux-tuple-incomplete` `launch-clock-discontinuity`; **measurement failures (advisor G2, never an empty set):** `tmux-panes-unreadable` `tmux-session-panes-unreadable` `pgrep-failed`; **state-text validation (G3, G4):** `generation-invalid` `clock-invalid`. tmux's own "no server running" is an absence, not a failure.
...
- **Advisor H1–H7 (identity-t5-review), built:** H1 `reobserve_same` — the adapter is asked again immediately before kill, spawn and close and must give the same answer and the same action key (claims 30a–f, queue shim); H2 the kill helper is executed as itself and must be `-x` (31); H3 the pending claim, the stop intent and the stop receipt gate their actions on a successful write (32; the receipt-rc mutation is detectable only by its message, because the same locked state directory also blocks the following claim write — source review + 32g); H4 `observe_line_valid` — one line, fifteen fields, closed vocabularies, structural invariants (33a–h); H5 the bridge library is sourced after `IS_CLAUDE`, for claude rows only, probed in a subshell (34; the fixture's broken library `exit`s); H6 in the observer — pid⇄birth and pane pid⇄birth are whole pairs, a stored nonce is 32 lowercase hex, `grep -F` (observer 20a–f); H7 the bind compares the whole record and its write gates the round; the claim is validated and written before attempts, marks and rename are armed (35a–e).
...
**Advisor J1–J7 (identity-t6-review):** J1 = H1–H7 above, built. **J2 — the host gate of spec §3 (desired vs other live rows' applied+pending in every readable home, refusing the write on conflict) is NOT in this task: it is Task 9, and Task 6 is NOT deployable until Task 9 is built.** J3 built — the rename step re-observes (`rename_same_process`) before judging the receipt and again immediately before typing (36a–d). J4 built — the pending baseline is a receipt: an absent or unreadable `pending_since` is re-seeded from the current nameSince and the round takes no further step, never coerced to 0 (37a–b); the applied write must succeed before anything is cleared or logged as receipted; `date` is validated before `applied_at` (37c). J5 built — the display is derived ONCE with stdout and stderr apart (a stderr file beside the state, read and removed). J6 built — `registry_session_rc_enabled` reads through `registry_load` (rc 78 on a row that does not load), exempts non-claude runtimes (rc 1) (display test 8d–8d4). J7 built — `type_line` returns 0/1/2; a failed literal is no attempt, a failed Enter is a loud partial attempt with no retry in the round (38a–e). The cycle now takes THREE rounds from a fresh row (seed, suspect, type).

**Advisor K1–K4 (identity-t56-final), built (c9b8e52, 885d0b2, f56f804):** K1 the last three readings before the keys are, in order, the fresh observation of the same process, the foreground, the OS birth (36e: the fixture flips the foreground during the round's THIRD observation, so only the order decides). K2 `rename_same_process` compares pid, birth, pane AND procStart before the receipt and before typing (36g–i). K3 `observe_line_valid` refuses answer/gen pairs the adapter cannot emit (no-process/wait-veto only beside none|bootstrap|gone-*, grace only beside grace; 33i–l). K4 `bridge_gen_matches_live` takes procStart as a fifth argument and a contradiction is not a match (generation 2h–2m); the observer passes the candidate's procStart (36a–c); `bind_generation` refuses to rebind the same pid+birth under another procStart, degraded once (35b reversed to a refusal). **K5 (identity-t56-contract), built:** an identified row is TYPED at the supervisor boundary — pid, procStart, nameSince, mtime and inode numeric and non-empty, birth `<token>:<digits>`, pane exactly `<NAME>:@<n>.%<m>`, sessionId non-empty — or it is `observer-unreadable` (33m–v: a foreign pane, an empty procStart, a superstring session name; nothing binds, receipts or types); `bridge_gen_matches_live` requires its fifth argument (the four-argument bypass is gone; generation 2j reversed, 3d). **K6 (identity-t56-contract, second pass):** the pane is parsed as `<id>:@<digits>.%<digits>` with both parts digit-checked AND rebuilt and compared whole, so `<id>:@0.%0.%1` and `<id>:@0.%0:@1.%1` are refused (33w–x; the two guards are individually redundant, jointly the property). **The validator moved to `lib/bridge.sh` as `bridge_line_valid` (BL_* fields)** so the supervisor, liveness-host and (mirrored in JS) the watch hold the line to ONE contract; proven in bridge-answer V1–V11.

### Task 7: Watch — the fifteen-field line, the remote socket built on the remote, kills only through `bridge-kill.py` (rc 65 → refuse, re-observe), info line on stderr asserted — as v4. `parseObserveLine` requires **fifteen** fields and the same vocabularies/invariants as the supervisor's `observe_line_valid`.

**Built locally (watch/), held uncommitted at the advisor's request until Tasks 5–6 are lockable.** `parseObserveLine` (one line, fifteen US fields, id equality, closed vocabularies, numbers, invariants); `decide()` takes `obs.identity` and makes no decision (no missing alarm, no action, latch untouched) for anything but `identified:managed` and `no-process`; `session-watch.mjs` asks the observer for every claude-code row (locally with the hub's state dir and socket, remotely with `STEWARD_STATE_DIR="$HOME/.local/state/<stateDirName>" STEWARD_TMUX_SOCKET="$HOME/.tmux/<tmuxSocket>" bash ~/scripts/bridge-observe.sh <id>` built on the remote), maps managed → the process by pid from ps (start, `--resume`) and addresses capture-pane, resume, reinject and the note to the bridge's exact pane; every other answer is one `[watch] identity <answer> for <id>` line on stderr; an unreachable host is unknown, not dead. `findProcess` (the label finder) is deleted. `restart-session.mjs` acts only on `identified:managed`, kills through `bridge-kill.py <pid> <birth> TERM` (rc 65 → refuse, say re-observe), polls the observer for a new managed pid with a fresh pane, resumes in that pane; other runtimes are refused. `registry-dump sessions` gains `runtime`. Tests: watch suite 183 (parse ×8, decide identity ×8, watch end-to-end: managed/no-process/unknown/grace/wait-veto/moved/malformed line/remote socket + unreachable, restart refusals ×5); eight mutations bite.

### Task 9: Gates (spec §3) — registry gate at write, host gate at spawn/rename — EXPANDED

**9a — registry gate (write time, estate-wide, static).** `lib/registry.sh`:
- `registry_display_of_fields <rc_label> <target_project> <target_entity> <slug>` — the desired display of a row that is not yet registered (the staged file's fields), with the same precedence as `registry_session_display` (label → project → entity → prefix+slug).
- `registry_session_login_key <login> <owner> <host>` → `LOGIN`, or `owner:<OWNER>@<HOST>` (the HOME) for a legacy row without one.
- `registry_session_rendered_unique <own-id> <login-key> <desired>` → prints the holder id, rc 0, when ANOTHER row under the SAME login key renders the same display; rc 1 when free. **Scoped per login key, not per estate — Jon's decision 2026-09-11 (point 4): two project accounts under different Claude logins may show the same RC name; each login has its own tile list. Within one login two identical names are two tiles nobody can tell apart.** The spec §3 sentence "two slugs rendering identically are refused" is read within a login. Scope within the key: every host and owner; claude-code rows only (RUNTIME first), RC-enabled (not `RC_LABEL=""`), not `retired`. A row never conflicts with itself; a row that does not render holds nothing. (First build was estate-wide and broke every fixture where two accounts target one project — the measurement that surfaced the scope.)
- `registry_session_work_rule <own-id> <login-key> <project>` → the holder, rc 0, when another non-retired claude-code row has the same login key and the same TARGET_PROJECT: "one claude-code conversation per (who pays, project)" on register lifecycle — never on live state, which a writer cannot measure.
- Wired INSIDE the write lock: `_registry_validate_session_stage` (bin/steward) computes the staged row's desired and refuses (rc 70, naming the holder and the display) on either collision; `linux/hub/enroll` applies `registry_session_rendered_unique` (key `owner:<login>@<host>`) to the label it is about to write, AFTER its same-home label gate, whose older message names the requester's remedy. Tests: `registry-session-add` (two slugs rendering identically → refused; same login+project → refused; a different project → ok; a retired holder does not reserve), `session-rc-label-unique` (a label carried in ANOTHER home is refused at enroll too).

**9b — host gate (spawn/rename time, local, measured).** Supervisor: `host_display_reserved <desired>` → the id, rc 0, when another generation in `STATE_DIR` (rows this uid owns) carries `applied` or `pending_for` equal to desired — or a live `bridge_name` equal to it (the additional collision guard) — AND that row is ACTIVE: its generation's pid alive with its birth (`bridge_os_birth`). Before `claude_claim_open` in a spawn: reserved → no spawn, rc 78, alarm once (`.display-reserved` marker, cleared when free). Before a rename step: reserved → no baseline, no typing, loud once. `STEWARD_RESERVATION_STRICT=1`: a row of ANOTHER owner on this host (register) whose rendered display equals desired is `uninspectable` and refuses with "manual census required"; unset, it is logged once. Tests in `supervisor-bridge`: a second claude row's generation with `applied` = desired and its pid alive → spawn refused, nothing typed; its pid dead → free; `pending_for` reserves too; the live bridge name reserves; strict mode refuses on another owner's rendered display, non-strict logs.

**9c — retarget refusal (spec §2 "retarget is not rename") — OUTLINE, separate build:** graph writers (`registry_entity_write`, `registry_project_write`) and the session retarget writers gate on register state only: a row is stopped when it carries `LIFECYCLE="stopped"` and `STOP_RECEIPT="<epoch>:<pid>:<birth>"`, written by a hub verb `registry session stopped <id> <pid> <birth>` that the supervisor's stop transaction requests over the bus after the last bridge file is gone; the reverse closure walks PARENT/MANAGED_BY up to 64 levels across all hosts' rows. `lib/registry.sh` never calls the observer.

**9a and 9b are BUILT, and rebuilt on the advisor's M1–M5 (identity-t9-review).**

- **The namespace is the LOGIN KEY, in the SPEC** (M1): the design document was amended (`1db1040`) rather than reinterpreted in the plan — `LOGIN`, or `owner:<OWNER>@<HOST>` for a legacy row. Both gates use it, so a pair the registry allows cannot be blocked by the host gate (claim 39q).
- **The staged row decides, not the caller** (M2): `registry_session_gate_fields` in `lib/registry.sh` holds both rules and is proven on its own fields (registry-session-display section 10) — RUNTIME first (OpenCode and Codex exempt from both), RC-free exempt from the rendered gate but still bound by the work rule, a row never gating against itself. `_registry_validate_session_stage` resets and reads `RUNTIME` and `RC_LABEL` from the staged file, so `migrate-session`, which preserves the old row's runtime through the same validator, is covered.
- **ACTIVE is a live bridge file** (M3): `host_display_reserved` asks the adapter about every same-key claude row on this host and counts only `identified:*`; then it compares that row's reported name and its generation's `applied`/`pending_for`. A generation whose pid is alive but that has no bridge file holds nothing. A row of another unix account is uninspectable — strict refuses, otherwise it is named once. The candidate's unix user is resolved through its ACCOUNT, not read off OWNER, which may be a principal.
- **Check and reserve are one critical section** (M4): a re-entrant host-wide lock (`<state>/.display-reservation.lock`, mkdir) spans the gate and the write it authorises, and the launch claim now writes `pending_for` — between the claim and the first observation there is no bridge file, so the claim is what holds the name. A lock another supervisor holds stands the round down; one older than `STEWARD_RESERVATION_STALE_SEC` (120) is broken loudly; a state directory that cannot be written is NOT reported as contention.
- **Fail closed** (M5): both predicates answer rc 2 *uninspectable* naming the row. A row the loader refuses is not omitted — it is read WITHOUT being executed (at schema 6 every row must name a LOGIN, so an estate mid-migration is full of such rows, and a gate that refused every write while one lay there would stop the migration it protects). Only a row that yields neither a login key nor a display is uninspectable. `REGISTRY_GATE_EXCLUDE` keeps the row a migration is about to consume out of its own gate.

Tests: registry-session-display 84 (sections 9 and 10), registry-session-add 102, registry-migrate-session 85, registry-session-derive 38, hub-enroll-conf 109, supervisor-bridge 239 (sections 39–42). Mutations bite for every guard above. 9c (retarget refusal) is not built.

### Task 8: `linux/liveness-host.sh` corroborates with the bridge — BUILT

A claude-code row's `agent` is the adapter's answer: `running` iff `identified:managed`, `not-running` iff `no-process`, otherwise `unknown` with the answer (and classes, generation) as the row's `reason`; the observer is asked with `STEWARD_STATE_DIR=$HOME/.local/state/<STATE_DIR_NAME>` and `STEWARD_TMUX_SOCKET=$HOME/.tmux/<TMUX_SOCKET>`, found beside the script (`STEWARD_BRIDGE_OBSERVE` overrides). The pane walk and the host's pgrep gate remain for OpenCode rows only; codex rows are untouched; `tmux` stays an independent measurement. A missing observer, a line that is not the fifteen-field contract, or a line about another id is `unknown` with its reason. `lib/liveness.sh` carries a session row's `reason` into the eighth field (was always `-`); `unknown` was already in the vocabulary. Tests: liveness-host 135, sessions-liveness 67 (+2). **Advisor L1/L2 (identity-t8-review), built:** L1 `observe_agent` validates through the shared `bridge_line_valid` — a fifteen-field `no-process` beside `alive`, an identified row with a foreign pane or an empty procStart are `unknown` with "breaks the contract", never a verdict; L2 a claude row never calls `list-panes` here (only OpenCode rows walk panes), so an unreadable pane census omits only the OpenCode row while the claude rows stay measured (tmux up from list-sessions, agent from the adapter). Three mutations bite (no pair check, panes for claude, the fixture's own untyped lines).

### Task 10: `linux/bridge-census.sh` — the one-time bootstrap census (P0) — BUILT

Runs as the owner over every claude-code row of this uid on this host (or one `<id>`), asks `bridge-observe.sh --bootstrap <id>`, validates the answer through `bridge_line_valid`, and seeds the generation: `identified:managed` → the verified record (pid, birth, procStart, sessionId, bridge name/nameSince/mtime/inode, uid) + `census=1`; `no-process` → `stop_receipt=census-<epoch>` + `census=1`; any other answer → `census=blocked:<answer>` (the supervisor keeps the row unknown until an operator resolves it and re-runs `--force <id>`); `not-applicable` and `uninspectable` are listed, nothing written; `account-missing` is listed as the P0 PREREQUISITE (the ACCOUNT migration first), nothing written. A row already carrying `census=1` is skipped unless `--force`; `--dry-run` writes nothing. rc 0 when every row this uid can seed is seeded, 1 when anything is blocked or a prerequisite is missing. The only writer of `census`. **Advisor N1–N3 (identity-t10-review), built:** N1 every `identified:*` answer seeds the verified record — managed, ORPHAN and MOVED alike; blocking an orphan would leave a live process the adapter can never match again, and the supervisor's own policy is what reaps it on the next round. N2 one eligibility gate (`census_eligible`) for one row and for all of them: an explicit `<id>` is resolved, loaded, and required to belong to this unix account (through its ACCOUNT, not its OWNER) and to this host — a foreign or unknown id is refused with no generation written and no observer call. N3 `--force` is a complete census, not a merge: `census_clear` removes every field the census does not itself write (identity, launch claim, pending name, spawn state) while keeping the generation's history of earlier processes, so a re-censused no-process row cannot carry a stale identity dressed as "seeded". Tests: 57 claims; eight mutations bite.

### Task 11: `steward registry session derive <session>` — the migration verb — BUILT

A row stops carrying a typed `RC_LABEL` and starts deriving its display from its target. **The line is DELETED, never emptied** (spec §4's trap: `RC_LABEL=""` is the RC-FREE choice in both readers, so emptying it would take every migrated session out of Remote Control silently). Refusals, all before anything is written and all leaving the row byte-identical: an RC-free row (a choice, not a migration), a row with no target, a target that does not render, and a rendered display already held under this row's login key (the Task 9a gate). `--dry-run` reports what would happen; `--json` is data; a row already without the line answers "already derives". After the rewrite the row must load AND render the promised display, or the backup is put back. **Advisor P1–P5 (identity-t11-review), built:** the rewrite goes through `registry_row_replace` — the registry's own transaction — so P1 the gate runs on the STAGED row inside the writer's lock (`registry_derive_validate_stage` in `lib/registry.sh`, proven directly in section 6: a competitor that takes the display between the pre-check and the publish is caught there), P2 the published row keeps mode 0600 under a loose umask and leaves no staging debris, and P5 a failed readback restores the previous row, with a failed restore its own loud failure. P3 RUNTIME first: an OpenCode or Codex row is refused before any gate or write, byte-identical. P4 a row already without the line whose target renders nothing is refused, not reported as an idempotent success. Tests: 57 claims; nine mutations bite — including the one this task exists for (empty instead of delete).
...
**P1b case B — MEASURED 2026-09-12 07:57–08:03 (basement, Point login, Jon's eyes): the tile is STALE after a resume with new argv; no duplicate.** The hub's own row: conf set to `Steward→Basement (P1b-B)`, its timer stopped (the deployed supervisor finds a claude by its label in argv and would have spawned a duplicate beside the live process), the sibling hub `s-68413113933455e1` as briefed backup. Timeline (local): `/exit` typed 07:57:44 by the backup, process gone 07:57:49 (clean exit, 5 s), deployed supervisor run by hand 07:57:56 — SILENT, no process (its zombie rule wants two consecutive rounds, the first says nothing), run again 07:58:20 — zombie pane cleaned, same thread resumed 07:58:21 with `--name` AND `--remote-control` both `Steward→Basement (P1b-B)`; the bridge file reports `(P1b-B)`, nameSource user, fresh nameSince. Jon's screenshot 08:03: **`Steward→Basement (Point)` — unchanged, one tile**. The tile was 32 s without a process behind it and did not disappear. **Reading:** the tile is keyed to the thread and carries its last confirmed name (first registration or `/rename`); a later `--remote-control` on resume does not move it. This is the spec's default assumption, now measured: *rename pending stays persistent from the last confirmed apply and the cycle runs regardless of what the new bridge file reports*. The supervisor is already consistent with it — the receipt needs the pane's `Session renamed to:` line and a nameSince above the baseline seeded AFTER the spawn, so a fresh bridge that merely reports the desired name is not a receipt and `/rename` is typed. **Production defect this measures:** every restart the deployed supervisor performs with a changed label leaves a stale tile (argv and bridge say the new name, the app shows the old).

**Runbook fact, both supervisors:** a no-process row is acted on only after two consecutive rounds, and the first is silent. A restart by hand is "run twice" or "timer on and wait one period", never "run once".
