...
- Produces:
  ...
  - `steward invite ls [--json]`. rc 0. Columns: `id principal name entity host runtime provider state expires`. Never a token. **rc 78 when the register cannot be read as a register**: `registry_invite_list` refuses an absent `invites.d`, and a row that is listed but does not load is reported and refused rather than dropped from the listing - a listing that quietly omits a row is a listing that answers "there is no such invitation" about one that exists.
...
- Produces: `steward invite redeem <token>|- |--token-file <path> --identity <source>:<value> [--email <e>] [--json]`.
    - **A program hands the token over out of argv; a human types it.** On Linux `/proc/<pid>/cmdline` is world-readable by default, and this verb runs on the host where every invited member has an account, so a positional token is a live one-time invitation link in the process table for as long as the redemption takes. `--token-file <path>` reads the token from the first line of a file and `-` reads it from one line on standard input; both refuse rc 64 when there is no token to read, and neither ever puts the value in a child's argv. The spec's positional form is unchanged and stays the one for a human at a terminal. **The Desk apply step (build-order step 4) MUST use `--token-file` or `-`.**
  - rc 0 when all twelve steps have left their mark; rc 64 for a malformed argument; rc 65 when the token matches no row, the row is not effectively `open`, the identity already maps to a principal, the invitation names a runtime the session writer cannot yet write, or the invitation names a unix account that already exists on this host and is not one this product may take over; rc 70 when a step fails; rc 77 when `sudo -n` cannot run the helper; **rc 78 for everything the register or the machine cannot answer** - no sha256 tool on PATH (or the one there failed), the invitation row does not load, the principal register cannot be searched for the identity, the estate names no state directory, the entity does not load, and no hub host or hub ssh target is named.
    - **`--` ends the flags.** `redeem -- <token> [more flags]` hands over a token the caller did not choose, for the tokens minted before the minter stopped producing a leading hyphen. Everything after `--` is a positional value, so `redeem -- TOKEN --identity oidc:a:b` is three tokens and rc 64 `one token at a time` - put the flags BEFORE the `--`.
    - **The helper's floor is asked about an account this verb did not create, too.** Step 3's mark is "a home exists", so an account already on the host skips the helper - and with it the four questions the helper asks about every account it makes: not `root`, not the account running the verb, uid at least 1000, and a home at `/home/<name>`. They are therefore asked again here, locally and without `sudo` (`id -u <name>` plus the home path the step already has), before step 3 branches: an existing account that fails any of them is rc 65 and nothing is written for it. Measured before that gate: an invitation naming an account somebody else made wrote a principal row and an account row for it, installed the hub's delivery key in its `~/.ssh/authorized_keys`, seeded its home and deployed the skeleton into it.
    - **"Created" is one of two answers the helper gives on rc 0, and the receipt must not confuse them.** `steward offboard` never deletes the passwd row, so a redemption for an offboarded principal finds a home path with nothing at it and calls the helper; the helper's already-exists branch configures nothing (no unlock, no lingering, no home). After the helper returns, the step therefore requires the home to EXIST - rc 70 naming the offboarded account as the usual cause - and prints `<u> created` only when the helper's output carries `helper: account <u> created`, `<u> already existed` otherwise.
...
- Produces: `steward offboard <principal> [--keep-home] [--json]`.
  - rc 0 when everything is removed (or already gone); rc 64 for a malformed argument; rc 70 when a step fails; rc 77 when `sudo -n` cannot run the helper; rc 65 when a row this verb must not break names the principal (an account on another host, a host row's `OWNER`/`OPERATOR` - every such host row is named, not just the first). rc 78 covers everything the register cannot answer before the first side effect: the principal has no row; an account row of theirs does not load (the row that names the unix account, so nothing here could lock it or move its home); an account row's `HOST` or `USERNAME` is a value the register's own writer would have refused; a register directory this verb reads exists and cannot be listed (an unreadable register is not an empty one); a register directory is a symlink that points at nothing (the estate said something is there and what it named is gone - not the same answer as a directory nobody ever made); a register directory this verb reads is something other than a directory (a plain file at a register's path is neither a register nor an absent one, and it survives a mode sweep while reading as "never set up"); the `entities.d` register is absent (see below); the third column of this verb's own register list is neither `required` nor `optional` (an unrecognised class is a bug in this file, and the permissive reading is the one a typo would silently select, so it is answered rather than walked past); or any `*.conf` inside one of those registers exists, or is a link to something that does not, and is not a regular file this run can read (`[ -r ]` alone is true for a directory and for a fifo, and a fifo named `*.conf` blocked the run forever on the first bare `source`, after the unix account was locked and before any receipt was written).
    - **Absent is legitimate for six registers, and refused for one.** `accounts.d`, `principals.d`, `sessions.d`, `logins.d`, `hosts.d` and `invites.d` may each be missing and the cleanup still finishes at rc 0 with a receipt in `state:"done"` - nothing that is not there can name anybody. `entities.d` may not: `registry_entity_list` refuses a directory that is not there, and the membership step that calls it runs long AFTER the unix account is locked and its account row removed, so an absent `entities.d` used to end the run at rc 78 with a half-offboarded person, a receipt in `state:"failed"`, and a second-run guard answering rc 70 for that principal until somebody finished it by hand. The register gate therefore carries a `required`/`optional` column and refuses that one absence up front, where refusing still costs nothing.
    - **The unreadable-row refusal is estate-wide, deliberately.** A row nobody can read may be anybody's, so a single unreadable `*.conf` in any of the seven registers this verb reads (`accounts.d`, `principals.d`, `sessions.d`, `logins.d`, `hosts.d`, `entities.d`, `invites.d`) refuses EVERY offboarding in that estate until the mode or the owner of that file is fixed - not only the offboarding of the person the file happens to name. The alternative is the measured behaviour it replaces: a mode-000 row was passed over at rc 0 with `state:"done"` and an empty `warnings[]`, leaving a second unix account unlocked, a bus relay row standing in the hub's `authorized_keys`, or a membership intact. A refusal an operator can clear with one `chmod` is the cheaper half of that trade.
  - One line per thing removed, on stdout, each `offboard: <what>`. A second run over the same principal prints `offboard: nothing left to remove` and exits 0 - but only if the receipt says `state:"done"`; a receipt in any other state is rc 70 naming the receipt, because the rows can be gone while what came after them is not.
  - A receipt file at `$HOME/.local/state/<STATE_DIR_NAME>/offboards/<principal>.receipt.json` (directory 0700, file 0600) with `{"schemaVersion":2,"principal":"...","state":"done|failed","at":<epoch>,"removed":[...],"kept":[...],"warnings":[...]}`. `removed` holds only what was actually removed or locked; `kept` holds what was deliberately left (the invitation as history, `--keep-home`, a session or login row the loader would not vouch for - an ACCOUNT row that will not load refuses the run instead, since it is the row that names the unix account); `warnings` holds what went wrong without stopping the run, and the `stopped here:` line of a run that did stop. The `--json` value carries the same three lists. A receipt that cannot be written is rc 70, not a silent success - it is the second-run guard.
